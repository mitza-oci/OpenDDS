...
namespace ICE {
  ...
  private:
    ...
    // 6.1.2.1
    bool is_running() const { return !is_completed() && !is_failed(); }

    bool is_completed() const { return m_nominated != m_valid_list.end(); }

    bool is_failed() const {
      return m_scheduled_for_destruction ||
        (m_valid_list.empty() && m_frozen.empty() && m_waiting.empty() && m_in_progress.empty());
    }

                              ...
                              OPENDDS_SET(ACE_INET_Addr)& addrs) const
{
  ...
  if (local_id == GUID_UNKNOWN) {
    for (iter_t iter = locators_.begin(); iter != locators_.end(); ++iter) {
      addrs.insert(get_address(iter->first));
    }
    return;
  }
...
{
  ACE_INET_Addr addr = get_address(remote_id);
  if (addr == ACE_INET_Addr()) {
...
void
RtpsUdpDataLink::send_ack_nacks(RtpsReaderMap::value_type& rr, bool finalFlag)
{
  using namespace OpenDDS::RTPS;
  ...
  for (WriterInfoMap::iterator wi = writers.begin(); wi != writers.end();
    ...
    if (wi->second.ack_pending_ || nack || finalFlag) {
      ...
      ACE_INET_Addr addr = get_address(wi->first);
      if (addr != ACE_INET_Addr()) {
        send_strategy()->send_rtps_control(mb_acknack, addr);
      } else {
  ...
  for (rw_iter rw = writers_.begin(); rw != writers_.end(); ++rw) {
    ...
    for (ri_iter ri = writer.remote_readers_.begin(); ri != end; ++ri) {
      ...
      if (!ri->second.requested_changes_.empty()) {
        ACE_INET_Addr addr = get_address(ri->first);
        if (addr != ACE_INET_Addr()) {
                                       ...
                                       OPENDDS_SET(ACE_INET_Addr)& gap_recipients)
{
  ...
  for (ri_iter ri = writer.remote_readers_.begin(); ri != end; ++ri) {
    ...
    ACE_INET_Addr remote_addr = get_address(ri->first);
    if (ri->second.requested_frags_.empty() || remote_addr == ACE_INET_Addr()) {
      continue;
    }
...
RtpsUdpDataLink::send_directed_nack_replies(const RepoId& writerId,
                                            ...
                                            ReaderInfo& reader)
{
  ACE_INET_Addr addr = get_address(readerId);
  if (addr != ACE_INET_Addr()) {
    return;
  }
...
RtpsUdpDataLink::send_heartbeats()
{
...

  {
    ...
    const ACE_Time_Value tv = now - 10 * config.heartbeat_period_;
    const ACE_Time_Value tv3 = now - 3 * config.heartbeat_period_;
    ...
    if (0 == std::memcmp(&it->writerId, &volatile_writer, sizeof(EntityId_t))) {
      ...
      if (rw != writers_.end()) {
        ...
        for (ReaderInfoMap::const_iterator ri = rinfo.begin();
          ...
          ACE_INET_Addr addr = get_address(ri->first);
          if (addr != ACE_INET_Addr()) {
            send_strategy()->send_rtps_control(*mb, addr);
          }
        }
      }

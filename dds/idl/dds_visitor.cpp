...
recursive_visitor::recursive_visitor(AST_Decl* scope)
  : scope_(scope)
{
}

int recursive_visitor::visit_scope(UTL_Scope* node)
{
  if (node->nmembers() > 0) {
    ...
    while (!si.is_done()) {
      d = si.item();
      if (d == 0) {
        ACE_ERROR_RETURN((LM_ERROR, "(%N:%l) recursive_visitor::visit_scope - bad node in this scope\n"), -1);
      }
      ...
      if (d->ast_accept(this) == -1) {
        ACE_ERROR_RETURN((LM_ERROR, "(%N:%l) recursive_visitor::visit_scope - failed to accept visitor\n"), -1);
      }
      si.next();
    }
  }
...
int recursive_visitor::visit_module(AST_Module* node)
{
  ...
  if (visit_scope(node) == -1) {
    ACE_ERROR_RETURN((LM_ERROR, "(%N:%l) recursive_visitor::visit_module - visit_scope failed\n"), -1);
  }
  return 0;
}

int recursive_visitor::visit_interface(AST_Interface* node)
{
  if (visit_scope(node) == -1) {
    ACE_ERROR_RETURN((LM_ERROR, "(%N:%l) recursive_visitor::visit_interface - visit_scope failed\n"), -1);
  }
  return 0;
}

int recursive_visitor::visit_exception(AST_Exception* node)
{
  if (node->imported()) {
    return 0;
  }
  if (visit_scope(node) == -1) {
    ACE_ERROR_RETURN((LM_ERROR, "(%N:%l) recursive_visitor::visit_exception - visit_scope failed\n"), -1);
  }
  return 0;
}
...
{
  if (visit_scope(node) == -1) {
    ACE_ERROR_RETURN((LM_ERROR, "(%N:%l) recursive_visitor::visit_root - visit_scope failed\n"), -1);
  }
  return 0;
}
...
codegen_visitor::codegen_visitor(AST_Decl* scope, bool java_ts_only)
  : recursive_visitor(scope)
  , error_(false)
  , java_ts_only_(java_ts_only)
{
...
int codegen_visitor::visit_root(AST_Root* node)
{
  error_ = false;
  gen_target_.gen_prologue();
  const int result = recursive_visitor::visit_root(node);
  gen_target_.gen_epilogue();
  return error_ ? -1 : result;
}

int codegen_visitor::visit_module(AST_Module* node)
{
  if (node->builtin()) {
    return 0;
  }
  ...
  return recursive_visitor::visit_module(node);
}

int codegen_visitor::visit_interface(AST_Interface* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_interface(node);
}

int codegen_visitor::visit_structure(AST_Structure* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_structure(node);
}

int codegen_visitor::visit_typedef(AST_Typedef* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_typedef(node);
}

int codegen_visitor::visit_enum(AST_Enum* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_enum(node);
}

int codegen_visitor::visit_interface_fwd(AST_InterfaceFwd* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_interface_fwd(node);
}

int codegen_visitor::visit_structure_fwd(AST_StructureFwd* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_structure_fwd(node);
}

int codegen_visitor::visit_constant(AST_Constant* node)
{
  const char* name = node->local_name()->get_string();
  BE_Comment_Guard g("CONST", name);

  AST_Decl* d = ScopeAsDecl(node->defined_in());
  const bool nested = d && (d->node_type() == AST_Decl::NT_interface);
  if (!java_ts_only_) {
    error_ |= !gen_target_.gen_const(node->name(), nested, node);
  }
  return recursive_visitor::visit_constant(node);
}

int codegen_visitor::visit_native(AST_Native* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_native(node);
}

int codegen_visitor::visit_union(AST_Union* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_union(node);
}

int codegen_visitor::visit_union_fwd(AST_UnionFwd* node)
{
  const char* name = node->local_name()->get_string();
  ...
  return recursive_visitor::visit_union_fwd(node);
}
